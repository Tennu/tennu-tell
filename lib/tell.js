...
function skip(nickname) {
    var self = this;
    return Promise.try(function() {

        var removed = _.remove(self.unreadTells, function(tell) {
            return tell.ToNick.toLowerCase() === nickname.toLowerCase();
        });
        ...
        var removedIDs = removed.map(function(tell) {
            return tell.ID;
        });

        // Mark tell read
        return self.knex('tell').whereIn('ID', removedIDs).update({
                Read: true
            })
            .then(function() {
                return removed;
